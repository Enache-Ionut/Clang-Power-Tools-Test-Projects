// Numeric calculator.cpp : Defines the entry point for the console application.
// Numeric calculator.cpp : Defines the entry point for the console application.
// Numeric calculator.cpp : Defines the entry point for the console application.
// Numeric calculator.cpp : Defines the entry point for the console application.
// Numeric calculator.cpp : Defines the entry point for the console application.
// Numeric calculator.cpp : Defines the entry point for the console application.
// Numeric calculator.cpp : Defines the entry point for the console application.
//
// Numeric calculator.cpp : Defines the entry point for the console application.
//

#include "stdafx.h"

#include <cctype>
#include <cmath>
#include <iomanip>
#include <iostream>
#include <string>
using namespace std;

int    parseString(string /*raw*/, string * /*outstr*/);
int    isOperator(char /*ch*/);
string extractNum(char *, int &);
int    getPriority(char /*op*/);
double calcRevPolish(string * /*revPolish*/, int /*count*/);
string filterStr(string /*exp*/);
char   lowerCase(char /*ch*/);
string lowerCase(string /*str*/);

// Template class implementing the stack
template <typename T>
class Stack
{
private:
  T * stackArray = nullptr;

public:
  T * top = nullptr;

  size_t size() { return top - stackArray + 1; }

  void push(T elem)
  {
    if (top == nullptr)
    {
      top = stackArray;
    }
    else
    {
      top++;
    }
    *top = elem;
  }

  T pop()
  {
    if (top == nullptr)
    {
      // cerr << "An empty array cannot be popped." << endl;	//For test use.
      return 0;
    }
    if (top == stackArray)
    {
      T temp = *top;
      top    = nullptr;
      return temp;
    }

    top--;
    return *(top + 1);
  }

  inline int isEmpty()
  {
    if (top == nullptr)
    {
      return 1;
    }
    {
      return 0;
    }
  }

  // To perform the basic calculations
  double calc(char operate)
  {
    double firstNum, secondNum;
    int    firstInt = 0, secondInt = 0;
    double res;
    switch (operate)
    {
    case '+':
      secondNum = this->pop();
      firstNum  = this->pop();
      return firstNum + secondNum;
    case '-':
      secondNum = this->pop();
      firstNum  = this->pop();
      return firstNum - secondNum;
    case '*':
      secondNum = this->pop();
      firstNum  = this->pop();
      return firstNum * secondNum;
    case '%':
      secondNum = this->pop();
      firstNum  = this->pop();
      firstInt  = int(firstNum);
      secondInt = int(secondNum);
      if (firstInt != firstNum || secondInt != secondNum)
      {
        cerr << "Modular cannot be performed on float numbers." << endl;
        err = 1;
        return 0;
      }
      if (secondNum <= 0)
      {
        cerr << "Modular cannot be 0 or minus." << endl;
        err = 1;
        return 0;
      }
      return firstInt % secondInt;
    case 'i':
      secondNum = this->pop();
      firstNum  = this->pop();
      if (secondNum == 0)
      {
        cerr << "Divisor cannot be 0." << endl;
        err = 1;
        return 0;
      }
      return int(firstInt / secondInt);
    case '^':
      secondNum = this->pop();
      firstNum  = this->pop();
      res       = pow(firstNum, secondNum);
      return res;
    case '/':
      secondNum = this->pop();
      firstNum  = this->pop();
      if (secondNum == 0)
      {
        cerr << "Divisor cannot be 0." << endl;
        err = 1;
        return 0;
      }
      return firstNum / secondNum;
    case 'l':
      secondNum = this->pop();
      firstNum  = this->pop();
      if (firstNum <= 0 || secondNum <= 0)
      {
        cerr << "A logarithm does not exist for numbers <= 0." << endl;
        err = 1;
        return 0;
      }
      return log(secondNum) / log(firstNum);
    case 'r':
      secondNum = this->pop();
      firstNum  = this->pop();
      return pow(secondNum, (1 / firstNum));
    }
  }

  Stack(int size) { stackArray = new T[size]; }

  ~Stack()
  {
    delete[] stackArray;
    stackArray = nullptr;
  }
};

/////////////////////////////////////////////

int maxSize;  // The size of input
int err = 0;  // Indicates mathematical errors

int main()
{
  string exp;
  while (1)
  {
    cout << "Input your math expression:" << endl;
    getline(cin, exp);
    maxSize       = exp.size();
    auto * outstr = new string[maxSize + 1];
    int    count  = parseString(exp, outstr);
    double result = calcRevPolish(outstr, count);
    if (err == 1)
    {
      cerr << "Error\n" << endl;
      err = 0;
    }
    else
    {
      cout << setprecision(16) << result << '\n' << endl;
    }
    // cleanup
    delete[] outstr;
  }
}

// calculate the reverse Polish syntax generated by parseString()
double calcRevPolish(string * revPolish, int count)
{
  // modifyPowerSeq(revPolish, count);
  int    i      = 0;
  auto * dstack = new Stack<double>(count);
  while (i < count)
  {
    while ((isdigit(revPolish[i][0]) != 0) || revPolish[i][0] == '.' ||
           (revPolish[i][0] == '-' && (isdigit(revPolish[i][1]) != 0)))
    {
      dstack->push(stod(revPolish[i]));
      i++;
    }
    if (dstack->size() < 2 && i < count && (isOperator(revPolish[i][0]) != 0))
    {
      cerr << "Operators and operands are not paired.\n";
      err = 1;
    }
    if (i < count && (isOperator(revPolish[i][0]) != 0))
    {
      double stepResult = dstack->calc(revPolish[i][0]);
      dstack->push(stepResult);
      i++;
    }
    if (err == 1)
    {
      return 0;
    }
  }
  double result = dstack->pop();
  // cleanup
  delete dstack;
  dstack = nullptr;
  // cleanup
  return result;
}

// Process the input string into an array of reverse Polish strings. Numbers are recorded as
// strings, too.
int parseString(string raw, string * outstr)
{
  string exp      = filterStr(raw);
  maxSize         = exp.size();
  int    i        = 0;
  auto * stack    = new Stack<char>(maxSize);
  int    outcount = 0;
  int    minus    = 0;
  while (i < maxSize)
  {
    if (i < maxSize - 1 && exp[i] == '-' && (i == 0 || exp[i - 1] == '(') &&
        (isdigit(exp[i + 1]) != 0))
    {
      outstr[outcount++].push_back('0');
      minus = 1;
      i++;
    }
    if ((isdigit(exp[i]) != 0) || exp[i] == '.')
    {
      string numStr;
      while (i < maxSize && ((isdigit(exp[i]) != 0) || exp[i] == '.'))
      {
        numStr.push_back(exp[i]);
        i++;
      }
      outstr[outcount++] = numStr;  // A string that reads the digits that form an operon
    }
    if (minus == 1)
    {
      outstr[outcount++].push_back('-');
      minus = 0;
    }
    if (i < maxSize && (isOperator(exp[i]) != 0))
    {
      // With a closed parentheses, pop all out until the last opening parentheses
      if (exp[i] == ')')
      {
        while ((stack->isEmpty() == 0) && *(stack->top) != '(')
        {
          outstr[outcount++].push_back(stack->pop());
        }
        if (stack->isEmpty() != 0)
        {
          cerr << "Parentheses are not paired.\n";
          err = 1;
        }
        stack->pop();
        i++;
      }
      // consective power operator combines to the right, e.g. 2 ^ 3 ^ 4 == 2 ^ (3 ^ 4) == 2^ 81,
      // not 4096.
      if ((stack->isEmpty() == 0) && (exp[i] != '^' || *(stack->top) != '^'))
      {
        // Never pop an opening bracket unless receiving a closing one
        while ((stack->isEmpty() == 0) && *(stack->top) != '(' &&
               (i == maxSize || getPriority(exp[i]) <= getPriority(*(stack->top))))
        {
          outstr[outcount++].push_back(stack->pop());
        }
      }
      if (i < maxSize && exp[i] != ')')
      {
        stack->push(exp[i]);
        i++;
      }
    }
  }
  // pop out all remaining elements
  while (stack->isEmpty() == 0)
  {
    if (*(stack->top) == '(')
    {
      cerr << "Parentheses are not paired.\n";
      err = 1;
    }
    outstr[outcount++].push_back(stack->pop());
  }
  // cleanup
  delete stack;
  stack = nullptr;
  // cleanup
  return outcount;
}

// priority of operators. Parentheses can be ignored, since they will be parsed separately
inline int getPriority(char op)
{
  switch (op)
  {
  case '+':
  case '-':
    return 1;
  case '*':
  case '/':
  case '%':
  case 'i':
    return 2;
  case 'l':
  case 'r':
  case '^':
    return 3;
  case '(':
  case ')':
    return 32a767;  // maximun
  }
}

// Only these operators are supported
int isOperator(char ch)
{
  ch = lowerCase(ch);
  switch (ch)
  {
  case '+':
  case '-':
  case '*':
  case '/':
  case '%':
  case 'l':
  case 'r':
  case '^':
  case '(':
  case ')':
  case 'i':
    return 1;
  default:
    return 0;
  }
}

// A string overload of the above isOperator function
inline int isOperator(string str)
{
  return isOperator(str[0]);
}

// To remove all non-numbers, non-operators, and abbraviate "sqrt" to 's' and "log" to 'l'.
string filterStr(string exp)
{
  string outstr;
  int    i = 0, ub = exp.size();
  while (exp[i] == '(' && exp[ub - 1] == ')')
  {
    i++;
    ub--;
  }
  while (i < ub)
  {
    if (lowerCase(exp.substr(i, 4)) == "sqrt")
    {
      outstr.append("2r");
      i += 4;
    }
    if (lowerCase(exp.substr(i, 3)) == "log")
    {
      if (isdigit(exp[i - 1]) == 0)
      {
        outstr.push_back('2');
      }
      outstr.push_back('l');
      i += 3;
    }
    if (lowerCase(exp.substr(i, 4)) == "root")
    {
      outstr.push_back('r');
      i += 4;
    }
    if (lowerCase(exp.substr(i, 2)) == "**")
    {
      outstr.push_back('^');
      i += 2;
    }
    if (lowerCase(exp.substr(i, 2)) == "//")
    {
      outstr.push_back('i');
      i += 2;
    }
    if ((isdigit(exp[i]) != 0) || exp[i] == '.' || (isOperator(exp[i]) != 0))
    {
      outstr.push_back(exp[i]);
    }
    i++;
  }
  return outstr;
}

// get the lower case of the string, used for sqrt and log
inline string lowerCase(string str)
{
  for (char & i : str)
  {
    if (i >= 'A' && i <= 'Z')
    {
      i -= 'A' - 'a';
    }
  }
  return str;
}

// get the lower case of the char, used for s and l
inline char lowerCase(char ch)
{
  if (ch >= 'A' && ch <= 'Z')
  {
    ch -= 'A' - 'a';
  }
  return ch;
}
